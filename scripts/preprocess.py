...
with engineered features suitable for both Random Forest and LSTM models.
"""
...
class CycleDataPreprocessor:
    ...
    def calculate_base_features(self, df):
        ...
        Args:
            df (pd.DataFrame): Raw data with start_date and end_date
    ...
    def create_lag_features(self, df, n_lags=None):
        ...
        This is the KEY transformation for time-series prediction:
        - X (features): Previous 3-5 cycle lengths and period durations
        - y (target): The cycle length we want to predict
        
        Args:
            df (pd.DataFrame): Data with base features
        ...
        df['cycle_length_mean_3'] = df['cycle_length'].shift(1).rolling(window=3, min_periods=1).mean()
        df['cycle_length_std_3'] = df['cycle_length'].shift(1).rolling(window=3, min_periods=1).std()
        
        df['period_duration_mean_3'] = df['period_duration'].shift(1).rolling(window=3, min_periods=1).mean()
    ...
    def prepare_supervised_dataset(self, df, target_col='cycle_length'):
        ...
        Returns:
            tuple: (X, y, feature_names) where X is features, y is target
        """
        # Drop rows with NaN (from lag features and last row with no next cycle)
        df = df.dropna()
        
        # Select feature columns (exclude dates and target)
        exclude_cols = ['start_date', 'end_date', 'cycle_length', 'days_since_last']
    ...
    def fit_transform(self, filepath, scale=True):
        ...
        Args:
            ...
            scale (bool): Whether to normalize features
            
        Returns:
            dict: Dictionary with X, y, dates, and metadata
        """
        # Load and process
    ...
    def transform(self, filepath, scale=True):
        """
...
def create_sample_data(output_path, n_cycles=36):
    ...
    This generates realistic cycle data with some natural variation:
    - Average cycle: 28 days (range: 25-32 days)
    - Average period: 5 days (range: 3-7 days)
    ...
    for i in range(n_cycles):
        ...
        cycle_length = max(25, min(32, cycle_length))  # Clip to realistic range
    ...
    print(f"Date range: {cycles[0]['start_date']} to {cycles[-1]['start_date']}")
...
def main():
    ...
    """
    # Setup paths
    raw_data_path = '../data/raw/cycles.csv'
    processed_data_path = '../data/processed/cycles_processed.npz'
    ...
    data = preprocessor.fit_transform(raw_data_path, scale=True)
